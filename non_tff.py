...
def run_non_tff(total_clients: int, clients_per_round: int, training_rounds: int, local_epochs: int, sampling_approach: str,
    ...
    # for priv_param in priv_lst:    # contains noise multipliers for dropout or epsilons for tightest
    ...
    client_lst = create_client_lst(data=(X_train, y_train), eps_lst=eps_lst, np_rng=np_rng, accountant_params=accountant_params,
                                   ...
                                   sampling_app=sampling_approach, clip_scheduler=clip_scheduler, dataset_name=dataset_name)  # , opt_schedule_params=opt_schedule_params)
    # , select_params, select_fn)
    ...
    print("\nFinished training")
    # training_metrics.append(server.get_metrics())
    # reject_metrics.append(server.get_rejections())

    # run_exp_dict['training_metrics'] = server.get_metrics()
    metrics_dict = server.get_metrics()
    # run_exp_dict['loss'] = metrics_lst[0]
    # for i in range(len(metrics)):
    #     run_exp_dict[metrics[i].name] = metrics_lst[i+1]
    ...
    # # clip_df_column = ['client_id', 'client_label', 'clip_scheduler_lst']
    # clip_df_column = ['client_label', 'clip_scheduler_lst', 'run_num']
    # if accountant_params['bucket_approach'] == 1:
    #     # clients in list of lists
    #     clip_df_row = []
    #     for bucket in client_lst:
    #         for client in bucket:
    #             clip_df_row.append(client.get_row())
    # else:
    #     clip_df_row = [client.get_row() for client in client_lst]
    # for client in clip_df_row:
    #     client.append(run_exp_dict['run#'])
    # print("\nclip df row", clip_df_row)
    # client_clip_df = pd.DataFrame(clip_df_row, columns=clip_df_column)
    # print("\nClient_clip_df", client_clip_df.to_string())

    return df_row


if __name__ == "__main__":
    run_non_tff(5, 2, 'mnist', [0.0, 0.5])
